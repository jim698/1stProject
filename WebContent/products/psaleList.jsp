...
<body>
	...
	<table>
		<tr>
		<c:forEach var='dto' items='${psaleList }' varStatus="status">
	...
	<div class="paging">
				<a href="${conPath }/psaleList.do?pageNum=1">&lt;&lt;</a>
				&nbsp; &nbsp; &nbsp;
				<c:if test="${startPage>BLOCKSIZE }">
					<a href="${conPath }/psaleList.do?pageNum=${startPage-1}">&lt;</a>
				</c:if>
				<c:if test="${startPage<=BLOCKSIZE }">
					&lt;
				</c:if>
				...
				<c:forEach var="i" begin="${startPage }" end="${endPage }">
					...
					<c:if test="${i != pageNum }">
						[ <a href="${conPath }/psaleList.do?pageNum=${i}">${i }</a> ]
					</c:if>
				</c:forEach>
				&nbsp; &nbsp; &nbsp;
				<c:if test="${endPage < pageCnt }">
					<a href="${conPath }/psaleList.do?pageNum=${endPage+1}">&gt;</a>
				</c:if>
				<c:if test="${endPage == pageCnt }">
					&gt;
				</c:if>
				&nbsp; &nbsp; &nbsp;
				<a href="${conPath }/psaleList.do?pageNum=${pageCnt}">&gt;&gt;</a>
			</div>	
	</section>
